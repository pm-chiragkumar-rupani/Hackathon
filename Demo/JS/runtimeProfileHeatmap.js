...
PubMatic.JobAnalysisProduct.Functions.getJobAnalysisRuntimeProfileHeatmapPanel = function(panelId, jobRuntimeProfileHeatmapDashboardData) {
	
	var element = "#" + panelId + "-body";
		
	var dataset = [];
	var margin, width, height, gridSize, legendElementWidth, svg, colors, colorScale, days, times, heatMap;
	
	margin = {top: 50, right: 10, bottom: 30, left: 45};
	...
	if(jobRuntimeProfileHeatmapDashboardData === '') {
		...
		heatMap = svg.append('g').attr('id', 'jobRuntimeHeatMapId');
		...
		dataset = jobRuntimeProfileHeatmapDashboardData.dataset;
		var maxValue = jobRuntimeProfileHeatmapDashboardData.maxValue;
		...
		heatMap = d3.select(element).select('svg').select('#jobRuntimeHeatMapId')
